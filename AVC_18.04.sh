#!/bin/bash
# AVC Masternode Setup Script V1 for Ubuntu 18.04 LTS
#
...
function stop_daemon {
    if pgrep -x 'AVCd' > /dev/null; then
        echo -e "${YELLOW}Attempting to stop AVCd${NC}"
        AVC-cli stop
        sleep 30
        if pgrep -x 'AVCd' > /dev/null; then
            echo -e "${RED}AVCd daemon is still running!${NC} \a"
            echo -e "${RED}Attempting to kill...${NC}"
            sudo pkill -9 AVCd
            sleep 30
            if pgrep -x 'AVCd' > /dev/null; then
                echo -e "${RED}Can't stop AVCd! Reboot and try again...${NC} \a"
                exit 2
            fi
        fi
    fi
...
echo -e "${GREEN} ------- AVC MASTERNODE INSTALLER v1.0.0--------+
 ...
 |        the masternode under a user AVC.         |::
 |                                                  |::
...
wget https://github.com/avacoins/AVCCoin/releases/download/v1.1/AVC-Daemon_Ubuntu_18.04.tar.gz
tar -xzvf AVC-Daemon_Ubuntu_18.04.tar.gz
...
======================================================================== \a"
echo -e "${GREEN}AVC_mn1 $publicip:$PORT $genkey TxId TxIdx${NC}"
echo -e "========================================================================
Use your mouse to copy the whole string above into the clipboard by
tripple-click + single-click (Dont use Ctrl-C) and then paste it 
into your ${GREEN}masternode.conf${NC} file and replace:
    ${GREEN}AVC_mn1${NC} - with your desired masternode name (alias)
...
cat ~/.AVC/AVC.conf
echo -e "${NC}-------------------------------------------------
NOTE: To edit AVC.conf, first stop the AVCd daemon,
then edit the AVC.conf file and save it in nano: (Ctrl-X + Y + Enter),
then start the AVCd daemon back up:
